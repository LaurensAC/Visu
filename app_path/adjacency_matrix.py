...
user_list = ['p1', 'p2', 'p3', 'p4', 'p5', 'p6', 'p7', 'p8', 'p9', 'p10',
...
colorSchemes = ['SteelBlue', 'Tomato', 'MediumSeaGreen', 'Inferno', 'Magma',
                'Plasma', 'Viridis']

# Set up widgets
city_select = Select(title="City", options=cities)
colorScheme_select = Select(title="Color Scheme", value="SteelBlue",
...
def update_data(attrname, old, new):
    ...
    for i in range(0, len(user_list)):
        for j in range(0, len(user_list)):
            value = adjacency[city][user_list[i]].get(user_list[j],
                                                      'Key not present')
            ...
            MappedFixationPointX.append(
                temp[(temp['user'] == user_list[i]) | (
                            temp['user'] == user_list[j])][
                    'MappedFixationPointX'])
    ...
    source4.data = dict(
        MappedFixationPointY=[item for sublist in MappedFixationPointY for item
                              in sublist],
        MappedFixationPointX=[item for sublist in MappedFixationPointX for item
                              in sublist]
    )
